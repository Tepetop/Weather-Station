...
class SDCard:
    ...
    def init_card_v2(self):
        ...
        for i in range(_CMD_TIMEOUT):
            time.sleep_ms(50)
            self.cmd(55, 0)
            arg = 0x40000000 if use_hcs else 0
            r = self.cmd(41, arg)
            if r == 0:
                # Default to block addressing; OCR (CMD58) can override for SDSC.
                self.cdv = 1
                if self.cmd(58, 0, -4) == 0:  # 4-byte response, keep first byte
                    ocr = self.tokenbuf[0]  # first OCR byte
                    if not ocr & 0x40:
                        # SDSC card, uses byte addressing in read/write/erase commands
                        self.cdv = 512
                # print("[SDCard] v2 card")
                return
            if r == (_R1_IDLE_STATE | _R1_ILLEGAL_COMMAND):
                # Some cards reject the HCS bit in ACMD41; retry without it.
                use_hcs = False
